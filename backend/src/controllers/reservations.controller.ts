...
export async function createReservation(req: Request, res: Response) {
  ...
  if (existingForVehicle) return res.status(400).json({ message: 'Vehicle already reserved' });
  const existingForUser = await Reservation.findOne({ userId, status: 'reserved' });
  if (existingForUser) return res.status(400).json({ message: 'User already has reservation' });
